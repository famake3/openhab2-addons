...
public class NinjaDoubleContactHandler extends BaseThingHandler implements NinjaThingEventListener {

    public final static Set<ThingTypeUID> SUPPORTED_THING_TYPES_UIDS = new HashSet<>(
            Arrays.asList(THING_TYPE_DOUBLE_CONTACT));
    ...
    public void ninjaEvent(int code) {
        ChannelUID channelUID = getThing().getChannel(NinjaBlocksBindingConstants.CONTACT).getUID();
        if (code == open_code) {
